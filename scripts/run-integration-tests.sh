...
# Run integration tests for app-fiap-videos-api.
# If DATABASE_URL is not set, starts a temporary Postgres container.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
CONTAINER_NAME="fiap-videos-api-integration-pg"
DB_URL="${DATABASE_URL:-postgresql://fiap:fiap@localhost:5432/fiap_videos_api_test}"
...
start_postgres() {
  if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
    ...
    docker run -d --rm \
      ...
      -e POSTGRES_DB=fiap_videos_api_test \
      -p 5432:5432 \
      postgres:16-alpine \
      >/dev/null
    ...
    for _ in {1..30}; do
      if docker exec "$CONTAINER_NAME" pg_isready -U fiap -d fiap_videos_api_test >/dev/null 2>&1; then
